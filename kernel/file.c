...
int mmap_handler(uint64 va, int cause){
  	int i = 0;
	struct proc *p = myproc();
	// try to find the vma which is corresponding to va
  	for(i = 0; i < NVMA; i++)
	...
	// use readi() to map the content of the file to the virtual address
	// the offset address of the mapping file is va - p->vma[i].addr
